...
{
    // Las expresiones ya deben ser booleanas (convertidas previamente)
...
{
    // Para operador NOT (!), el operando ya debe ser booleano
    if (t->lexeme == "!" && expr->type != ExprType::BOOL)
    {
        stringstream ss;
...
TypeConversion::TypeConversion(int target_etype, Expression *e) 
    : Expression(NodeType::TYPE_CONVERSION, target_etype, e->token), expr(e) 
{
    // No hay verificación de tipos aquí, ya se hizo antes
}
