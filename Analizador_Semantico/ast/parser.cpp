...
Statement *Parser::Stmt()
{
    ...
    case Tag::IF:
    {
        ...
        // CONVERSIÓN AUTOMÁTICA en contexto de condición
        cond = CheckBoolContext(cond);
    ...
    // { stmt.n = new While(bool.n, stmt₁.n); }
    case Tag::WHILE:
    {
        Match(Tag::WHILE);
        if (!Match('('))
...
Expression *Parser::Bool()
{
    ...
    //       | empty
    //
    // ACCIÓN SEMÁNTICA CON CONVERSIONES:
    // bool.n = new Log('|', bool₁.n, join.n)
    // donde bool₁.n y join.n deben ser convertidos a bool si son int
    ...
    while (true)
    {
        ...
        if (Match(Tag::OR))
        {
            ...
            // CONVERSIONES AUTOMÁTICAS según gramática
            // Aplicar CheckBoolContext a ambos operandos
            expr1 = CheckBoolContext(expr1);
            expr2 = CheckBoolContext(expr2);
...
Expression *Parser::Join()
{
    ...
    //
    // ACCIÓN SEMÁNTICA CON CONVERSIONES:
    // join.n = new Log('&', join₁.n, equality.n)
...
Expression *Parser::Unary()
{
    ...
    //
    // ACCIÓN SEMÁNTICA CON CONVERSIONES:
    // unary.n = new Unary('!', unary₁.n)
    // donde unary₁.n debe ser convertido a bool si es int
    ...
    if (lookahead->tag == '!')
    {
        ...
        // CONVERSIÓN AUTOMÁTICA para operador NOT
        expr = CheckBoolContext(expr);
...
{
    // Si la expresión ya es booleana, no hacer nada
    if (e->type == ExprType::BOOL)
        return e;
    
    // Si la expresión es entera, crear conversión automática a bool
    // Regla: 0 -> false, cualquier otro valor -> true
    if (e->type == ExprType::INT)
    {
        return new TypeConversion(ExprType::BOOL, e);
    }
    
    // Si es float, también convertir (opcional, siguiendo C/C++)
    if (e->type == ExprType::FLOAT)
    {
        return new TypeConversion(ExprType::BOOL, e);
    }
    
    // Si es void u otro tipo, mantener como está
    return e;
}
